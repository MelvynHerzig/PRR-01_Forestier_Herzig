...
func ServersSync(listener net.Listener) {
	...
	noServer := tcpserver.GetServerNumber()
	...
	for otherServ := noServer + 1 ; otherServ < uint(len(configReader.GetServers())) ; otherServ++ {
		var conn net.Conn
		var err error
...
func handleServer(conn net.Conn) {
	...
	for input.Scan() {
		mutex.HandleMessage(sync.Deserialize(input.Text()))
	}
}
...
func SendToAll(message string) {
	for i, _ := range connections {
		if i != int(tcpserver.GetServerNumber()) {
